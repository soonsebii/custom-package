...
#define AUTO_MAKENODE   1
//TODO
// 현재 상황은 class, device create 호출 정도의 수준에서 마무리.
// 추후 필요에 따라 소스 보강이 필요하다.
...
static int rdm_init(void)
{
    ...
    rdm_class = class_create(THIS_MODULE, "rdm0");
    result = PTR_ERR(rdm_class);
    if (IS_ERR(rdm_class)) {
        printk(KERN_WARNING "ps: can't create class file\n");
    }

    dev = MKDEV(rdm_major, 0);

    if (NULL == device_create(rdm_class, NULL, dev, NULL, "rdm0")) {
        printk(KERN_WARNING "ps: can't create device file\n");
    }
#endif
